...
class HostHandle(SShHandler):
    ...
    def printLine(self, page=1, offset=15):
        pmax = self.Hlen%15
        page = page if page <=pmax else pmax
        f = (page-1) * offset + 1
        l = page * offset + 1
        ...
        tmsg = tails.format('[%s]' %(self.Hlen%15),'','[%s]'%page)

        clear = os.system('clear')
        self.colorMsg(m=msg, c="white", title=True)

        for key in range(f,(l if < self.Hlen else self.Hlen):
            k = self.hinfo[key - 1]
            h = " {0} {1} {2}@{3}:{4} ".format(
                self.colorMsg(c="yellow", flag=True).format(
                    "{0: >4}".format("[%s]" % str(key))),
