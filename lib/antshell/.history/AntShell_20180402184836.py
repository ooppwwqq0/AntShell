...
class HostHandle(SShHandler):
    ...
    def print_hosts(self):
        ...
        for key in range(1, Hlen+1):
            k = self.hinfo[key-1]
            h = " {0} {1} {2}@{3}:{4} ".format(
                self.colorMsg(c="yellow", flag=True).format("{0: >4}".format("[%s]" %str(key))),
                self.colorMsg(c="green", flag=True).format("{0: <15}".format(k["name"])),
                self.colorMsg(c="green", flag=True).format("{0: >10}".format(k["user"])),
                self.colorMsg(c="green", flag=True).format("{0: >15}".format(k["ip"])),
                self.colorMsg(c="green", flag=True).format("{0: <5}".format(k["port"])),
            )
            rem = key % mode
