...
class HostHandle(SShHandler):
    ...
    def searchHost(self, include=None, pattern=False, match=False):
        ...
        search = self.__getSearch(include, match)
    ...
    def __getSearch(self, include, match, search=""):
    ...
    def _getInfo(self, ip):
        ...
        info["sudo"] = 0 if self.info["user"] == "root" else 1
        return info
    ...
    def addList(self):
        ...
        if len(res) == 1:
            ...
            sql = """insert into hosts(name,ip,user,passwd,port,sudo)
                values('{name}','{ip}','{user}','{passwd}',{port},{sudo})""".format(
                **self.info)
            self.db.execute(sql)
            self._commit()
        else:
    ...
    def hostLists(self, hlist=None):
        ...
        for key in range(1, Hlen + 1):
            ...
            info_mes = "{0}  {1} - {2}".format(
                self.colorMsg(c="red", flag=True).format(
                    "{0: >7}".format("[%s]" % str(key))),
                self.colorMsg(c="blue", flag=True).format(
                    "{0: <16}".format(h["name"])),
            ...
            if mode == 1:
                ...
                print(TailLine.format("|" + info_mes.ljust(
                    33 + (mode - rem - 1) + (mode - rem + 1) * count)))
            elif rem == 1:
